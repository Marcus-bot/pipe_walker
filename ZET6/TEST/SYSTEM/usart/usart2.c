...
u8 UART2_Receive(void)
{
	...
	if(USART2_RX_STA&0x8000)
	{
		for (i=0;i<16;i++)
		{
			if (USART2_RX_BUF[i]=='A'&&USART2_RX_BUF[++i]=='A')
				Start_flag=1;	//开始数据内容
			if (Start_flag==1)
			{
				for (j=0,++i;USART2_RX_BUF[i]!='B';i++)
				{
					Con_Info[j]=USART2_RX_BUF[i];
					j++;
				}
			if (USART2_RX_BUF[i]=='B'&&USART2_RX_BUF[++i]=='B')
				Start_flag=0;	//数据内容接受完成
			}
		}
	}
	USART2_RX_STA=0;
